...
module "container-pod-metric-alerts" {
  ...
  alarm_actions             = each.value.priority == "Critical" ? var.critical_alarm_actions : each.value.priority == "Warning" ? var.warning_alarm_actions : var.info_alarm_actions
  alarm_description         = "[${upper(each.value.priority)}] ${each.value.statistic} ${each.value.metric_name} of the pod ${var.pod_name} in namespace ${var.namespace} in cluster ${var.cluster_name} ${each.value.comparison_operator} ${each.value.threshold} within last ${each.value.evaluation_periods} ${each.value.period} second periods"
  application               = var.application
  tags                      = var.default_tags
  project                   = var.project
  region                    = var.region
...
module "container-service-metric-alerts" {
  ...
  alarm_actions             = each.value.priority == "Critical" ? var.critical_alarm_actions : each.value.priority == "Warning" ? var.warning_alarm_actions : var.info_alarm_actions
  alarm_description         = "[${upper(each.value.priority)}] ${each.value.statistic} ${each.value.metric_name} of the service ${var.service_name} in namespace ${var.namespace} in cluster ${var.cluster_name} ${each.value.comparison_operator} ${each.value.threshold} within last ${each.value.evaluation_periods} ${each.value.period} second periods"
  application               = var.application
  tags                      = var.default_tags
  project                   = var.project
  region                    = var.region
