...
def get_tag_value(tags, key):
    # Tag values are set by whoever created the resource, so strip control
    value = tags_to_dict(tags).get(key, "")
...
def sanitize_cell(value):
    ...
    # Newlines and NULs would break the row/column structure of the CSV.
    text = text.replace("\r", " ").replace("\n", " ").replace("\x00", "")

    if len(text) > MAX_CELL_CHARS:
        text = text[:MAX_CELL_CHARS] + "...(truncated)"

    # Prefixing with an apostrophe makes the spreadsheet read the cell as text.
...
def generate_csv(findings):
    ...
    for finding in findings:
        writer.writerow({name: sanitize_cell(finding.get(name)) for name in fieldnames})
